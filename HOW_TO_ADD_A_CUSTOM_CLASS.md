...
2. Add the new character icon to `/images/icons/characters/custom/`
3. Add the new character ability card images to `/images/character-ability-cards/custom/CLASS_CODE/`
4. (Optional) Add the new character mat front & back to `/images/character-mats/custom/`
5. (Optional) Add the new character perk sheet to `/images/character-mats/custom/`
...
Here is an example [pull request](https://github.com/cmlenius/gloomhaven-card-browser/pull/57) for this step

### Adding JSON data

1. Fork a branch from the `main` branch
2. Append the character ability card data to `data/character-ability-cards/custom.json`
...
```json
  {
    "name": ability name",
    "expansion": "Custom",
    "image": "character-ability-cards/custom/XX/ability-name.jpeg",
    "level": "1",
    "initiative": "72"
  }
```
...
```json
{ "id": "XX", "game": "custom", "colour": "#16395A", "base": true, "name": "Brute" }
```

Here is an example [pull request](https://github.com/cmlenius/gloomhaven-card-browser/pull/58) for this step
